...
#
# Usage:
#   powershell -ExecutionPolicy Bypass -File .\launcher\AceCombat2Launcher.ps1
...
#     %APPDATA%\AceCombat2Recomp\launcher-config.json (per-user). Falls back
...
[CmdletBinding()]
param(
    [string]$RecompExeName = 'Ace_Combat_2_Recompiled.exe'
)
...
function Get-ConfigDir {
    ...
    $dir = Join-Path $appData 'AceCombat2Recomp'
...
$form.Text = 'Ace Combat 2 Launcher'
...
$lblInfo.Text = "Select your legally obtained Ace Combat 2 disc image (.cue).`r`nChoose OpenBIOS (bundled) or your own legally obtained PS1 BIOS. Click Launch Game to start."
...
$btnBrowse.Add_Click({
    ...
    $dlg.Title = 'Select your legally obtained Ace Combat 2 disc image'
...
function Show-Error {
    ...
    [System.Windows.Forms.MessageBox]::Show($form, $Message, 'Ace Combat 2 Launcher', 'OK', 'Warning') | Out-Null
}
...
$btnLaunch.Add_Click({
    ...
    if (-not (Test-Path $cuePath)) {
        Show-Error "The selected CUE file does not exist:`n$cuePath"
        return
    }
    ...
    if ($useCustom) {
        ...
        if (-not (Test-Path $customBiosPath)) {
            Show-Error "The selected BIOS file does not exist:`n$customBiosPath"
            return
        }
    ...
    try {
        ...
        Show-Error "Failed to launch the game:`n$_"
        return
    }
    $form.Close()
})
